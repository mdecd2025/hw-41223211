...
# 啟動 HTTP 服務（端口 8000）
def run_http():
    app.run(host='2001:288:6004:17:fff1:cd25:0000:b024', port=8000, debug=True, use_reloader=False)

# 啟動 HTTPS 服務（端口 8443）
def run_https():
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
    context.load_cert_chain(certfile='localhost.crt', keyfile='localhost.key')  # 加載證書和私鑰
    app.run(host='2001:288:6004:17:fff1:cd25:0000:b024', port=8443, debug=True, ssl_context=context, use_reloader=False)
